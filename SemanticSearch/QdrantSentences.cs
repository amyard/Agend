...
public static class QdrantSentences
{
    internal static async Task Run()
    {
        ...
        var searchParam = new SearchParams { Exact = false, HnswEf = 128 };
        
        IReadOnlyList<ScoredPoint> query1 = await RunQuery(client, collectionNameCosine, query);
        IReadOnlyList<ScoredPoint> query2 = await RunQuery(client, collectionNameCosine, query, new SearchParams { Exact = false, HnswEf = 32 });
        IReadOnlyList<ScoredPoint> query3 = await RunQuery(client, collectionNameCosine, query, new SearchParams { Exact = false, HnswEf = 128 }); // default
        IReadOnlyList<ScoredPoint> query4 = await RunQuery(client, collectionNameCosine, query, new SearchParams { Exact = false, HnswEf = 512 });
    ...
    private static async Task<IReadOnlyList<ScoredPoint>> RunQuery(QdrantClient client, string collectionName, string query, SearchParams searchParams)
    {
        IEmbeddingGenerator<string, Embedding<float>> embeddingGenerator =
            new OllamaEmbeddingGenerator(new Uri("http://localhost:11434"), modelId: "all-minilm:22m");
