...
public class QdrantServiceProd
{
    private readonly QdrantClient _client;
    private readonly IEmbeddingGenerator<string, Embedding<float>> _embeddingGenerator;
    private readonly ulong _vectorSize;
    private readonly string[] _payloadIncludeFields = ["id", "title", "content"];
    private readonly string[] _payloadExcludeFields = [];

    // check the size which return embedding ollama model
    public QdrantServiceProd(string host, int port, Uri embeddingUri, string modelId = "all-minilm:22m", ulong vectorSize = 384)
    {
    ...
    public async Task<UpdateResult> PopulateCollectionAsync(string collectionName, IEnumerable<ContentInfoProd> contents)
    {
